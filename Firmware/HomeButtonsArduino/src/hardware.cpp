...
bool HardwareDefinition::init() {
  ...
  strncpy(model_name_, "Home Buttons", sizeof(model_name_) - 1);
...
bool HardwareDefinition::_efuse_burned() {
  // only the first 8 bits (1 byte) of the serial number field are read
  uint8_t buf[1] = {0};
  ESP_ERROR_CHECK(
...
void HardwareDefinition::_nvs_2_efuse() {
  ...
  bool success = true;
  // maxLen must be the destination buffer size, not the eFuse field width in
  // bits - Preferences only rejects values longer than maxLen.
  success =
      success && preferences.getString("serial_number",
                                       factory_params_.serial_number,
                                       sizeof(factory_params_.serial_number));
  success = success && preferences.getString("random_id",
                                             factory_params_.random_id,
                                             sizeof(factory_params_.random_id));
  success = success && preferences.getString("model_id",
                                             factory_params_.model_id,
                                             sizeof(factory_params_.model_id));
  success =
      success && preferences.getString("hw_version", factory_params_.hw_version,
                                       sizeof(factory_params_.hw_version));
  if (success) {
